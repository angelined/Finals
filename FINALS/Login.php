...
if (isset($_POST['btn_Create_new'])){
    ...
    foreach ($PDO->query($sql) as $row){
        ...
        $num_Costumer_no=(int)(substr($last_Costumer_no, -3))+1;   // yang -3 sa costnum hal. 0101 ang tada si 3 tas a + 1 kaya may +1 sa dulo
        $new_Costumer_no = sprintf('%04d',$num_Costumer_no);       // sprintf %04d para sa zero, kung ilan
    }
...
if(isset($_POST['btn_Submit'])){
    ...
    if ($editmode == "NewRecord"){ 
        ...
        $sql = "insert into costumerinfo values(?,?,?,?,?,?,?,?,?,?);";   // yang question mark is para sa entity na walang laman kasi lalagyan palang/
        $q = $PDO->prepare($sql);
...
if(isset($_POST['btn_edit'])) {
    ...
    $data = $q -> fetch(PDO::FETCH_ASSOC);   // tig rereturn niya si gabos na result sa row
